...
class ConfigService:
    ...
    def _load_session_config(self) -> None:
        ...
        try:
            ...
            if not user_session_file.exists():
                ...
                logger.info(f"Created default user session config at: {user_session_file}")
    ...
    def _fetch_server_config(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to fetch server config. Status code: {response.status_code}")
    ...
    def _merge_session_with_server_config(self) -> None:
        ...
        for section in self._session_config:
            if section in merged and isinstance(self._session_config[section], dict) and isinstance(merged[section], dict):
                # Recursively merge this section
                merged[section] = self._merge_configs(merged[section], self._session_config[section])
            else:
                # Direct override
    ...
    def get(self, path: str, default: Any = None) -> Any:
        ...
        try:
            ...
            if path.startswith("model.") or path.startswith("speech.") or path.startswith("hearing.") or path == "agent_id":
